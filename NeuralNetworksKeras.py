...
print("The softmax action function was use for the output layer because the output of the softmax activation function provides a probability distribution value as output. This provides an output value which is easier to interpret as value is always between 0 to 1") 
...
print("The size of the first hidden layer is 300. None means the size can be variable.\
The total number of parameters of the first hidden layer is 235500, which refers to the number of weights and biases")
model.summary()
...
print("An epoch is a process where a dataset is iterated over on a complete cycle. This is done as a part of trainiing a machine learniing model. During the iteration, the losses are calculated and the weights are optimized based on the training data provided.")
